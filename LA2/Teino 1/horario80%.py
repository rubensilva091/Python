...
"""
ucs1 = {"la2": ("quarta",16,2), "pi": ("terca",15,1), "cp": ("terca",14,2),"so": ("quinta",9,3)}
alunos1 = {5000: {"la2","cp"}, 2000: {"la2","cp","pi"},3000: {"cp","poo"}, 1000: {"la2","cp","so"}}
...
def horario(ucs,alunos):    
    ...
    for k1,tup1 in ucs.items():
        for k2,tup2 in ucs.items():
            if (k1!=k2):
                ...
                if((hora_final1>=tup2[1] and hora_final1<=hora_final2) or (hora_final2>=tup1[1] and hora_final2<=hora_final1)):
                    flag=1
                if(flag and tup1[0]==tup2[0]):
                    if ((k1,k2) not in sobreposicao_ucs and (k2,k1) not in sobreposicao_ucs):
    ...
    for (k,v) in alunos.items():
        ...
        if (flag1):
            ...
            for uc1 in v:
                ...
                #Pormenor : A CADEIRA EXISTIR NAS UCS. por exemplo poo nao existe nas UCS, logo nao pode ser considerado
    ...
    #retornar o tuplo
    return sorted(alunos_com_todas_ucs)

print(horario(ucs3, alunos3))
